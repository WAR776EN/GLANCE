...
exports.apply = function(req, res) {
  companyModel.findOne({'general.name':req.body.name}, function(err, data){
    if (err){
      res.send(err);
      return err;
    }
    for (let id of data.aplicants){
      var isExist = false;
    ...
    if (!isExist) {
      data.aplicants.push(req.decoded._id);
  ...
  companyModel.findOne({'general.name':req.body.name})
    ...
    .exec(function(err, data){
      ...
      userModel.findByIdAndUpdate(req.decoded._id, {$push:{applied:data._id}}, function(err){
        if (err){
          res.status(400);
